...
class GPML_MCMC_NN():
    ...
    def fit(self,X_Xtimes_dataset,epochs=1000,learning_rate=0.001):
        ...
        for epoch in range(epochs):
            #batches
            for batch_X,batch_time in X_Xtimes_dataset:
                ...
                print(f"epoch={epoch}/{epochs} | batch_loss={batch_loss.data[0]}")
            #showing off model fit
    ...
    def show_fit(self,params):
        ...
        print(f"tau | true={params['taus'].data},fit={fitted_params['taus'].data} | {params['taus'].data-fitted_params['taus'].data}")
        print(f"kernal_signal_sd | true={params['kernal_signal_sds'].data},fit={fitted_params['kernal_signal_sds'].data} | {params['kernal_signal_sds'].data-fitted_params['kernal_signal_sds'].data}")
        print(f"kernal_noise_sd | true={params['kernal_noise_sds'].data},fit={fitted_params['kernal_noise_sds'].data} |  {params['kernal_noise_sds'].data-fitted_params['kernal_noise_sds'].data}")

def unvec_embed(theta):
    def embed(z):
        out_arr = torch.stack([theta*z,theta*z**2])
        return out_arr
    return embed

(X_train,X_times_train,z_train),(X_test,X_times_test,z_test),params = nn_embedding_dataset(device,train_num=10,test_num=1)
dataset = TensorDataset(X_train,X_times_train)
batched_data = DataLoader(dataset,batch_size=2)

model = GPML_MCMC_NN(device,1,2)
model.fit(batched_data,epochs=1)
print(model.dataset_loss(X_train,X_times_train))
